...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_IMAGE && resultCode == Activity.RESULT_OK) {
            ...
            try {
                ...
                fileAsset = new File(getCacheDir(),"test.conf");
                OutputStream outpu = new FileOutputStream(fileAsset);
                byte[] buffer = new byte[4 * 1024];
                int read;

                while((read = inputStream.read(buffer)) != -1){
                    outpu.write(buffer,0,read);
                }
                outpu.flush();
            } catch (IOException e) {
                e.printStackTrace();
    ...
    public void takePicture() {
        ...
        Uri uri = FileProvider.getUriForFile(this,this.getApplicationContext().getPackageName(),destination);
